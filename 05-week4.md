...
- 从 HuggingFace 下载 Qwen3-4B（小模型方便迭代）
- 用 transformers 加载权重
- **不**用 transformers 的 forward（自己写）
...
## Day 23：PagedAttention 实现（5月24日）
...
def paged_attention_decode_kernel(
    ...
    # 参考 vLLM 中 paged_attention_v1 kernel
    ...
```
...
## Day 24：Continuous Batching Scheduler（5月25日）
...
### 晚上（1h）：跑 hello world
```python
engine = LLMEngine("Qwen/Qwen3-4B")
...
## Day 25：MLA Backend + FlashAttention 接入（5月26日）

### 上午（4h）：MLA backend

**为简历加分：手写 MLA backend**
...
### 下午（4h）：接入真 FlashAttention v3/v4
...
### 晚上（1h）：测试
- 用 mini-vllm 跑 DeepSeek-V3-Distill-Qwen-7B（MLA 模型）
- 对比输出与 vLLM 是否一致

### Checkpoint
- [ ] MLA backend 跑通至少 1 个 DeepSeek 模型
- [ ] FA backend 集成完成

---

## Day 26：Benchmark 对标 vLLM + 项目完善（5月27日）
...
# 对比 vLLM
mini_t = bench(MiniVLLMEngine("Qwen3-4B"), prompts)
vllm_t = bench(VLLMEngine("Qwen3-4B"), prompts)
print(f"Mini-vLLM: {mini_t:.2f}s, vLLM: {vllm_t:.2f}s, ratio: {mini_t/vllm_t:.1%}")
```

**目标：达到 vLLM 30-50% 性能**（这已经非常体面了）
...
A from-scratch LLM inference engine with PagedAttention, MLA, and Continuous Batching.

## ✨ Features
- ✅ PagedAttention with block-level KV cache
- ✅ Continuous batching + chunked prefill
- ✅ MLA (Multi-head Latent Attention) for DeepSeek models
- ✅ FlashAttention v4 backend (Blackwell SM 10.0)
...
- 项目部分：Mini-vLLM + vLLM PR + NVFP4 量化模型 + 4 篇博客
...
✅ **最低标准：**
1. Mini-vLLM 能跑通（哪怕只 batch=1）
2. 投递 ≥10 家
3. 至少 1 场技术面（不限通过）

🎯 **理想：**
1. Mini-vLLM 跑出 vLLM 30%+ 性能
2. 3+ 家公司技术面
3. 1 个 offer
