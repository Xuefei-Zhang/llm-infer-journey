...
1. **读面试题** → 先看当天 Day 的 `🎯 今日面试题`，明确今天要掌握什么概念、要能回答什么问题
2. **读参考代码** → 去 `~/3rd/vllm` 或 `~/3rd/nano-vllm` 找对应代码实现（可用 deepwiki 等分析工具），结合面试题理解概念。理解"为什么这样设计"比"怎么实现"更重要
3. **回答面试题** → 用自己的话回答今日面试题，AI 纠正/补充
4. **手工作业** → 理解到位后，才开始当天任务的 coding/experiment
5. **讨论纠错** → 做完后 AI 用费曼技巧反向检查（规则 4）
6. **写每日笔记** → ≤30 行记录到 `progress.md`

> ⚠️ 不要在我还没回答面试题时就急着让我写代码。理解先行，动手在后。
...
AI 应该说：
✅ "好，按 02-week1.md 的 Day 3 计划。
今日面试题是 C.24（Tensor Core）。
先去看 ~/3rd/vllm 的对应代码或 PMPP 第 9 章，理解 wmma/wgmma。
看完回答：为什么 Blackwell 引入 wgmma 替代 wmma？
